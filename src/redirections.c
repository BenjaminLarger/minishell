...
/*   Updated: 2024/03/12 12:44:30 by blarger          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (!ft_strcmp(args[0], "|"))
		msg_kill_free(TOKEN, get_linker(args[0]), EXIT_FAILURE, data);
...
/* !!! When doing a pu2, program act as if ctrl D was pressed */
static int	process_input(char **args, t_minishell *data)
{
	if (is_linker(args[1]) == TRUE)
		msg_kill_free(TOKEN, get_linker(args[1]), EXIT_SUCCESS, data);
...
static int	process_here_doc(char **args, t_minishell *data)
{
	...
	if (is_linker(args[1]) == TRUE)
		msg_kill_free(SYNTAX, get_linker(args[1]), EXIT_SUCCESS, data);
	if (pipe(pipefd) == -1)
		perror("pipe error:");
	while (1)
	{
